...
func GenCrypto(domainName string, orgName string, numOrgs int, numPeer int, numOrderer int) (Config, error) {
	...
	peerData, err := GenPeerConfig(domainName, orgName, numOrgs, numPeer)
	check(err)
...
func GenPeerConfig(domainName string, orgName string, numOrgs int, numPeers int) ([]OrgSpec, error) {
	config := []OrgSpec{}
	...
	for i := 0; i < numOrgs; i++ {
		...
		tempconfig := OrgSpec{
			Name:     orgName + strconv.Itoa(i+1),
			Domain:   orgName + strconv.Itoa(i+1) + "." + domainName,
