...
func GenConfigtx(domainName string, orgName string, numOrgs int, numOrderer int, numKafka int) (TopLevel, error) {
	...
	for i := 1; i <= numOrgs; i++ {
		temporg, _ := GenOrg(i, domainName, orgName)
	...
	topOrg = append([]*Organization{}, org...)
	// topOrderer := &orderer
...
func GenOrg(orgId int, domainName string, orgName string) (Organization, error) {
	orgIdStr := strconv.Itoa(orgId)
	anchor := AnchorPeer{
		Host: "peer0." + orgName + orgIdStr + "." + domainName,
		Port: 7051,
	}

	org := Organization{
		Name:        orgName + orgIdStr + "MSP",
		ID:          orgName + orgIdStr + "MSP",
		MSPDir:      "crypto-config/peerOrganizations/" + orgName + orgIdStr + "." + domainName + "/msp",
